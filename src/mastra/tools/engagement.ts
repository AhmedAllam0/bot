  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      ...
      if (userResult.rows.length === 0) {
        return {
          success: false,
          message: "❌ لم يتم العثور على حسابك. جرب تسجيل الدخول اليومي (/checkin) أولاً.",
        };
      }
...
• سجل دخولك اليومي (/checkin) ✅ (+5 نقاط)
• تفاعل في الجروب 💬 (+2 لكل رسالة)
• ادعُ أصدقاءك (/referral) 🔗 (+50 نقطة)`,
          };
        }
      }
